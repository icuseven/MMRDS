function user_render(p_metadata, p_data, p_ui, p_metadata_path, p_object_path, p_is_grid_context)
{
	...
	switch(p_metadata.type.toLowerCase())
  {
    case 'grid':
		...
		for(var i = 0; i < p_data.length; i++)
		{
			...
			for(var j = 0; j < p_metadata.children.length; j++)
			{
				...
				result.push("<td>");
				Array.prototype.push.apply(result, user_render(child, p_data[i][child.name], p_ui, p_metadata_path + ".children[" + i + "]", p_object_path + "[" + i + "]." + child.name, is_grid_context));
				result.push("</td>");
			}
    ...
    case 'group':
		...
		{
			var child = p_metadata.children[i];
			Array.prototype.push.apply(result, user_render(child, p_data[child.name], p_ui, p_metadata_path + '.children[' + i + "]", p_object_path + "." + child.name));
		}
	...
	case 'datetime':
		...
		case 'time':
			...
			break;
     default:
          console.log("user_render not processed", p_metadata);
       break;
  }
