...
func _input(event):
		...
		if event.button_index == MOUSE_BUTTON_RIGHT: # && event.shift_pressed:
			...
			if event.pressed:
				...
				if selection_is_dragging:
					...
					for item:GroupField in SelectedItemList.keys():
						var target_object = event_queue.target_object.new()
	...
	if event is InputEventKey and event.pressed:
		...
		if event.is_action_pressed("ui_undo"):
			if event_stack.stack:
				var last_event = event_stack.pop()
				for object in last_event.target_object_list:
					var item = instance_from_id(object.object_id) 
					item.move_to(object.from_position)
