...
namespace mmria.services.model.actor.quartz
{
    public class Process_DB_Synchronization_Set : UntypedActor
    {
        ...
        protected override void OnReceive(object message)
        {
            ...
            switch (message)
            {
				...
				mmria.services.model.couchdb.c_change_result latest_change_set = get_changes (Program.Last_Change_Sequence, scheduleInfo);
				...
				{
					foreach (mmria.services.model.couchdb.c_seq seq in latest_change_set.results)
					{
					...
					(
						new Action (async () => 
						{
							if (kvp.Value.Value)
							{
								try
								{
									mmria.services.util.c_sync_document sync_document = new mmria.services.util.c_sync_document (kvp.Key, null, "DELETE");
							...
							else
							{
								...
								try
								{
									...
									{
										mmria.services.util.c_sync_document sync_document = new mmria.services.util.c_sync_document (kvp.Key, document_json);
										await sync_document.executeAsync ();
									}
				...
				try
				{
					...
					string json = null;
					mmria.services.model.couchdb.c_all_docs all_docs = null;
					cURL curl = null;
					...
					json = curl.execute ();
					all_docs = Newtonsoft.Json.JsonConvert.DeserializeObject<mmria.services.model.couchdb.c_all_docs> (json);
					foreach (mmria.services.model.couchdb.c_all_docs_row all_doc_row in all_docs.rows)
					{
        ...
        public mmria.services.model.couchdb.c_change_result get_changes(string p_last_sequence, ScheduleInfoMessage p_scheduleInfo)
        {

			mmria.services.model.couchdb.c_change_result result = new mmria.services.model.couchdb.c_change_result();
			string url = null;
			...
			result = Newtonsoft.Json.JsonConvert.DeserializeObject<mmria.services.model.couchdb.c_change_result>(res);
