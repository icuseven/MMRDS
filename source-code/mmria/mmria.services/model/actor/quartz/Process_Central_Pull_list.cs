                        ...
                        for (var i = 0; i < cdc_instance_pull.Length; i++)
                        {
                            ...
                            if(db_name_split.Length > 1)
                            {
                                ...
                                foreach(var case_response_item in case_response.rows)
                                {
                                    ...
                                    var de_identified_json = new mmria.services.util.c_cdc_de_identifier(document_json).executeAsync().GetAwaiter().GetResult();
                                    ...
                                    if(result.ok)
                                    {
                                        var Sync_Document_Message = new mmria.services.model.actor.Sync_Document_Message
                                        (
                                            _id,
                                            de_identified_json
                                        );

                                        Context.ActorOf(Props.Create<mmria.services.model.actor.Synchronize_Case>()).Tell(Sync_Document_Message);
                                    }
