...
/*
			...
			try
			{
				...
				var user_curl = new mmria.services.cURL("GET", null, request_string, null, config_timer_user_name, config_timer_password);
            ...
            if(user == null)// if user does NOT exists create user with email
            {
                ...
                try
                {
                    ...
                    var user_curl = new mmria.services.cURL("PUT", null, user_db_url, object_string, config_timer_user_name, config_timer_password);
                    var responseFromServer = await user_curl.executeAsync();
