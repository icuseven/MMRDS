        ...
        public async Task Setup()
        {
            ...
            ) 
            {
                ...
                ) 
                {
                    ...
                    mmria.services.model.couchdb.c_change_result latest_change_set = Newtonsoft.Json.JsonConvert.DeserializeObject<mmria.services.model.couchdb.c_change_result> (res);

                    Program.Last_Change_Sequence = latest_change_set.last_seq;



                    var Sync_All_Documents_Message = new mmria.services.model.actor.Sync_All_Documents_Message
                    (
                        DateTime.Now
                    );

                    _actorSystem.ActorOf(Props.Create<mmria.services.model.actor.Synchronize_Case>()).Tell(Sync_All_Documents_Message);
                    ...
                    //{
                        ...
                        /*
                        var Process_All_Migrations_Message = new mmria.services.model.actor.quartz.Process_Initial_Migrations_Message
                        (
                            DateTime.Now
                        );

                        _actorSystem.ActorOf(Props.Create<mmria.services.model.actor.quartz.Process_Migrate_Data>()).Tell(Process_All_Migrations_Message);
                        */
