...
namespace mmria.services.util
{
  public class mmrds_exporter
  {
    ...
    private mmria.services.model.actor.ScheduleInfoMessage Configuration;

    public mmrds_exporter(mmria.services.model.actor.ScheduleInfoMessage configuration)
    {
      this.Configuration = configuration;
      //this.is_offline_mode = bool.Parse(Configuration["mmria_settings:is_offline_mode"]);

    }
    public bool Execute(mmria.services.export_queue_item queue_item)
    {
      ...
      try
      {
        ...
        //mmria.services.util.c_de_identifier.De_Identified_Set = de_identified_set;

        List<System.Dynamic.ExpandoObject> all_cases_rows = new List<System.Dynamic.ExpandoObject>();


        var jurisdiction_hashset = mmria.services.util.authorization.get_current_jurisdiction_id_set_for(this.juris_user_name);
        ...
        foreach (System.Dynamic.ExpandoObject case_row in all_cases_rows)
        {
          ...
          if (home_record != null)
          {
            ...
            foreach (var jurisdiction_item in jurisdiction_hashset)
            {
              ...
              if (regex.IsMatch(home_record["jurisdiction_id"].ToString()) && jurisdiction_item.ResourceRight == mmria.services.util.ResourceRightEnum.ReadCase)
              {
        ...
        mmria.services.util.cFolderCompressor folder_compressor = new mmria.services.util.cFolderCompressor();
