...
public sealed class v3_8_Migration
{
	...
	{
		this.output_builder.AppendLine($"v3.8.Migration Data Migration started at: {DateTime.Now.ToString("o")}");
		DateTime begin_time = System.DateTime.Now;
		
		this.output_builder.AppendLine($"v3_8 Migration started at: {begin_time.ToString("o")}");
		...
		try
		{
			...
			foreach(var kv in id_record_id_tuple)
			{
				...
				if(doc != null)
				{
					...
					var mmria_case_id = value_result.result.ToString();
...
/// ADD Appropriate data migration

				if(!is_report_only_mode && case_has_changed)
				{
					var save_result = await new SaveRecord(this.host_db_url, this.db_name, this.config_timer_user_name, this.config_timer_value, this.output_builder).save_case(doc as IDictionary<string, object>,"v3.9 Migration");
				}
	...
	Console.WriteLine($"v3_8_Migration Finished {DateTime.Now}");
}
