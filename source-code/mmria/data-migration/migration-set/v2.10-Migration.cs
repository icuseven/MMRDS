...
public sealed class v2_10_Migration
{
	...
	{
		this.output_builder.AppendLine($"v2.10 Data Migration started at: {DateTime.Now.ToString("o")}");
		DateTime begin_time = System.DateTime.Now;
		
		this.output_builder.AppendLine($"v2_10_Migration started at: {begin_time.ToString("o")}");
...

{
	...
	)
	{
		if( !isInNeedOfConversion(time_value_string))
		{
			...
			check_and_update_muilti_value("birth_certificate_infant_fetal_section/record_identification/time_of_delivery");
				...
				{
					var save_result = await new SaveRecord(this.host_db_url, this.db_name, this.config_timer_user_name, this.config_timer_value, this.output_builder).save_case(doc as IDictionary<string, object>,"v2.10 part1");
				}
	...
	Console.WriteLine($"v2_10_Migration Finished {DateTime.Now}");
}
