...
public sealed class GA_One_Time
{
    ...
    {
        this.output_builder.AppendLine($"GA_One_Time Data Migration started at: {DateTime.Now.ToString("o")}");
        DateTime begin_time = System.DateTime.Now;
        
        this.output_builder.AppendLine($"GA_One_Time started at: {begin_time.ToString("o")}");
...
/*
2	Record Linkage of Death and Birth/Fetal Death Certificates	
3	Record Linkage of Death Certificate and Hospital Discharge Data



Record linkage of death and birth certificate

Record linkage of death certificate hospital discharge data
...
*/
                ...
                try
                {
                    ...
                    var how_was_this_death_identified = "home_record/how_was_this_death_identified";
                    value_result = gs.get_value(doc, how_was_this_death_identified);
                    if (!value_result.is_error)
                    {
                        ...
                        {
                            if(value_result.result.ToString().ToLower() == "Record linkage of death and birth certificate".ToLower())
                            {
                            ...
                            else if(value_result.result.ToString().ToLower() == "Record linkage of death certificate hospital discharge data".ToLower())
                            {
                                if(case_change_count == 0)
                                {
                                    case_change_count += 1;
    ...
    Console.WriteLine($"GA_One_Time Finished {DateTime.Now}");
}
