...
if(theValue) {
document.cookie = name + '=' + escape(value) + ((path)?';path=' + path:'') + ((domain)?';domain=' + domain:'') + ((secure && (secure == true))?'; secure':''); // set an already-expired cookie -- Actually set it to no time, ie a Session Cookie
}
} // WM_killCookie

function customResearch3214994(){
var n = 100; /* One out of n visitors will be given survey */
var random_num;
var offset = -1;
var timespent;
var cookiename = "NetRakerRID3214994";
...
if ((random_num == 1) && (navigator.appName.indexOf('WebTV') == -1)) 
  if (!WM_readCookie(cookiename)) { 
   ...
   WM_setCookie(cookiename,'3214994') 
   window.open( 
    "/flu/survey.htm", 
    "Custom_Research", 
