...
DragSelect.prototype._startUp = function (event) {
  ...
  // this.selectorPosition = this.getPosition(event);
...
// DragSelect.prototype._isAClick = function (newPosition) {
//   const threshold = 5;
//   const startPosition = this.selectorPosition;
//   const dx = Math.abs(startPosition.x - newPosition.x);
//   const dy = Math.abs(startPosition.y - newPosition.y);
//   // return true if the change is less than the threshold
//   return dx + dy < threshold;
// };

DragSelect.prototype._handleMove = function (event) {
  var selectorPos = this.getPosition(event); // callback
  // if (this._isAClick(selectorPos)) {
...
DragSelect.prototype.removeSelectables = function (
  _nodes,
  removeFromSelection
) {
  var nodes = this.toArray(_nodes);
...
DragSelect.prototype.isElement = function (node) {
  try {
    ...
    return (
      _typeof(node) === 'object' &&
      node.nodeType === 1 &&
      _typeof(node.style) === 'object' &&
      _typeof(node.ownerDocument) === 'object'
    );
  }
};
/**
