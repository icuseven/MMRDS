...
// global for interact.js
let interactJSActive = false;
...
formDesigner = {
  ...
  fdObjectHandler: {
    snapShot: function (undo = false, init = false, unwrap = false) {
      if (undo) {
        $('.fd-path-object').removeAttr('style');
        return true;
      }
    ...
    quickSnap: function () {
      ...
      uiSpecification.localRevision[uiSpecification.localCount] = JSON.parse(
        JSON.stringify(uiSpecification.currentObject)
      );
      $('#local-rev-container').html(
        fdTemplates.uiSpecification.dashboard.localRev()
      );
    },
    rollBackRevision: function () {
      ...
      uiSpecification.currentObject = JSON.parse(
        JSON.stringify(
          uiSpecification.localRevision[uiSpecification.localCount]
        )
      );
  ...
  uiSpecHandler: {
    ...
    writeToModal: function () {
      let modalSpecObject = JSON.stringify(
        uiSpecification.currentObject,
        undefined,
        4
      );
    ...
    deleteSpec: function () {
      $.ajax({
        url:
          apiURL +
          endpointUISpecification +
          uiSpecification.currentID +
          '?rev=' +
          uiSpecification.currentObject._rev,
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        data: '',
        type: 'DELETE',
      ...
      $.ajax({
        ...
        .done(function (response) {
          ...
          if (response_obj.ok) {
            ...
            ma.innerHTML =
              'save completed SUCCESSFULLY @ ' + new Date().toISOString();
          }
        })
        .fail(function (xhr, status, error) {
          var ma = document.getElementById('fd-messages');
    ...
    getSpec: function (currentID) {
      $.get(apiURL + endpointUISpecification + currentID).done(function (
        data,
        status
      ) {
        uiSpecification.currentObject = data;
        uiSpecification.localRevision[0] = JSON.parse(
          JSON.stringify(uiSpecification.currentObject)
        );
        ...
        if (
          uiSpecification.currentObject._id === null ||
          uiSpecification.currentObject._id === 'default_ui_specification'
        ) {
  ...
  canvasHandler: {
    ...
    formFields: {
      ...
      setFormGroupPosition: function (targetID) {
        let fgp = {
          top: Math.min.apply(null, [
            $(`label[for= '${targetID}']`).offset().top,
            $(`#${targetID}`).offset().top,
          ]),
          left: Math.min.apply(null, [
            $(`label[for= '${targetID}']`).offset().left,
            $(`#${targetID}`).offset().left,
          ]),
        };
        return fgp;
      },
      wrapLabelControl: function (stacked = true) {
        ...
        $.each(elems, function (index, value) {
          if (value.tagName === 'LABEL') {
            targetID = $(value)[0].htmlFor;
            let fgp = formDesigner.canvasHandler.formFields.setFormGroupPosition(
              targetID
            );
            $(`label[for= '${targetID}'], #${targetID}`).removeAttr('style');
            $(`label[for= '${targetID}'], #${targetID}`).wrapAll(
              `<div class="form-group form-group-wrapper" />`
            );
          }
        });
        if (stacked) {
          formDesigner.canvasHandler.formFields.widestInFormGroup();
        }
        ...
        $.each(fields, function (index, value) {
          switch (value.type.toLowerCase()) {
            ...
            case 'textarea':
              tpl.push(fdTemplates.formFields.prompt(formName, value));
              tpl.push(
                fdTemplates.formFields.controls.textarea(formName, value)
              );
              break;
            case 'button':
              tpl.push(fdTemplates.formFields.prompt(formName, value));
  ...
  if (selected_item_list.length > 1) {
    ...
    for (var i = 1; i < selected_item_list.length; i++) {
      ...
      if (
        ...
        selected_item_list[i].style.top =
          previous_item.offsetTop +
          previous_item.offsetHeight +
          new Number(p_field_pixels) +
          'px';
      } else {
        selected_item_list[i].style.top =
          previous_item.offsetTop +
...
function align_width_space_of_selection(p_pixels) {
  ...
  if (selected_item_list.length > 1) {
    ...
    for (var i = 1; i < selected_item_list.length; i++) {
      ...
      selected_item_list[i].style.left =
        previous_item.offsetLeft +
        previous_item.offsetWidth +
        new Number(p_pixels) +
        'px';
    }
  }
    ...
    for (var i = 1; i < selected_item_list.length; i += 2) {
      ...
      if (
        ...
        current_item.style.left =
          previous_item.offsetLeft + previous_item.offsetWidth + 15 + 'px';
      }
    }
  }
