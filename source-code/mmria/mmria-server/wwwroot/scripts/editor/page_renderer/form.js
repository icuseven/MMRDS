function form_render(
  ...
  if (
    ...
    enable_edit_disable_attribute = ' disabled '; //disable enable edit btn
    currently_locked_by_html =
      '<i>(Currently Locked By: <b>' + g_data.last_checked_out_by + '</b>)</i>'; //show user locked info
  }
  ...
  if (p_metadata.cardinality == '+' || p_metadata.cardinality == '*') {
    ...
    // The 'Records' Table
    p_result.push(
      '<div class="construct__table row no-gutters search_wrapper">'
    );
      ...
      for (let key in g_data) {
            ...
            p_result.push(`
                                    <li>
                                        <p class="mb-2 font-weight-bold">
                                            Reviewer's Notes from <a href="${noteUrl}/${i}">Case Form</a>: Record ${
              i + 1
            }
                                            ${
                                              !isNullOrUndefined(recordType)
                                                ? `– ${recordType}`
                                                : ''
                                            }
                                            ${
                                              !isNullOrUndefined(birthOrder)
                                                ? `– ${birthOrder}`
                                                : ''
                                            }
                                            ${
                                              !isNullOrUndefined(timeOfDelivery)
                                                ? `– ${timeOfDelivery}`
                                                : ''
                                            }
                                        </p>
        ...
        //~~~ informant_interviews
          ...
          for (let i = 0; i < notes.length; i++) {
            ...
            function validate(...mmddyy) {
              for (let i = 0; i < mmddyy.length; i++) {
                currentTimeUnit = mmddyy[i];
                if (!currentTimeUnit || currentTimeUnit === '(blank)')
                  return false;
              }
              return true;
            }
            p_result.push(
              `<li>
                ...
                i + 1
                      ...
                      ${validate(interviewType) ? `– ${interviewType}` : ''}
                      ${
                        validate(relationshipToDeceased)
...
function render_print_form_control(p_result, p_ui, p_metadata, p_data) {
  function createOpt(value, display, record) {
    const dataRecord = record ? ` data-record="${record}"` : '';
    p_result.push(`<option value="${value}"${dataRecord}>${display}</option>`);
  }
  if (parseInt(p_ui.url_state.path_array[0]) >= 0) {
    ...
    p_result.push(
      '<select id="print_case_id" class="form-control" onChange="print_case_onchange(event)">'
    );
    ...
    if (!isNaN(path_to_check_multi_form)) {
      ...
      const display =
        ...
        ')';
      createOpt(p_metadata.name, display, path_to_check_multi_form + 1);
    } else {
      // check if it's Multi Forms or not.
      const all = p_data && isNaN(path_to_check_multi_form) ? 'All ' : '';
      createOpt(p_metadata.name, 'Print ' + all + p_metadata.prompt);
    }
    p_result.push('</optgroup>');

    p_result.push('<optgroup label="Other">');
    createOpt('core-summary', 'Print Core Elements Only');
    createOpt('all', 'Print All Case Forms');
    p_result.push('</optgroup>');

    p_result.push('</select>');
    p_result.push('<button>Print Form</button>');
  }
}
