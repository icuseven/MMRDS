function datetime_render(p_result, p_metadata, p_data, p_ui, p_metadata_path, p_object_path, p_dictionary_path, p_is_grid_context, p_post_html_render, p_search_ctx, p_ctx)
{
	/*
			...
			p_result.push(">");
				...
				p_result.push(`
					<div class="row no-gutters datetime-control" style="${style_object && get_style_string(style_object.control.style)}" dpath="${convert_object_path_to_jquery_id(p_object_path)}">
				...
				let id_path = convert_object_path_to_jquery_id(p_object_path);
				...
				p_post_html_render.push(`
					toggle_disabled($('#${id_path} .datetime-date'), $('#${id_path} .datetime-time'));
				`);

				p_post_html_render.push(`
					$('#${id_path} .datetime-date').on('change', (event) => {
						let target = $(event.target).parent().next();
						let new_date_value = $('#${id_path} .datetime-date').val();
						let new_time_value = $('#${id_path} .datetime-time').val();
						
						//if new_date_value exists
						if (!isNullOrUndefined(new_date_value))
						{
							//we update the singular (actual) control with new vals
							target.val( new_date_value + ' ' + new_time_value );
						}
				...
				p_post_html_render.push(`
					$('#${id_path} .datetime-time').on('change', (event) => {
						let target = $(event.target).parent().next();
						let new_date_value = $('#${id_path} .datetime-date').val();
						let new_time_value = $('#${id_path} .datetime-time').val();
						
						//if new_date_value exists
						if (!isNullOrUndefined(new_time_value))
						{
							//we update the singular (actual) control with new vals
							target.val( new_date_value + ' ' + new_time_value );
						}
