async function render13(p_post_html)
{
...
${render_navigation_strip(13)}
<div>
<h3>${metadata.title}</h3>
<p>${metadata.description}</p>
<ul style="align:center;">
    <li style="display: inline-block;float:left;width:48%">
        <div align=center>${await render13_chart(p_post_html, metadata, data_list)}</div>
        <br/>
        <div align=center>${await render13_table(metadata, data_list)}</div>
        </div>
...
async function render13_chart(p_post_html, p_metadata, p_data_list)
{
...
async function render13_table(p_metadata, p_data_list)
{
    const totals = new Map();
