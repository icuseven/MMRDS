async function render7(p_post_html)
{
    const metadata = indicator_map.get(7);
    const data_list = await get_indicator_values(metadata.indicator_id);

    const metadata2 = indicator_map.get(7.2);
...
<ul style="align:center;">
    <li style="display: inline-block;float:left;width:48%">
        <div align=center>${await render71_chart(p_post_html, metadata, data_list)}</div>
        <br/>
        <div align=center>${await render71_table(metadata, data_list)}</div>
        </div>

    </li>

    <li style="display: inline-block;float:right;width:48%">
        <div align=center>${await render72_chart(p_post_html, metadata2, data_list2)}</div>
        <br/>
        <div align=center>${await render72_table(metadata2, data_list2)}</div>
        </div>
...
async function render71_chart(p_post_html, p_metadata, p_data_list)
{
...
async function render72_chart(p_post_html, p_metadata, p_data_list)
{
    const totals = new Map();
...
async function render71_table(p_metadata, p_data_list)
{
    const totals = new Map();
...
async function render72_table(p_metadata, p_data_list)
{
    const totals = new Map();
    const name_to_title = new Map();
