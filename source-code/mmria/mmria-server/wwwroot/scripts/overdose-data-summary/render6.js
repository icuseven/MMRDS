async function render6(p_post_html)
{
    ...
    const metadata2 = indicator_map.get(6.2);
...
<ul style="align:center;">
    <li style="display: inline-block;float:left;width:48%">
        <div align=center>${await render61_chart(p_post_html, metadata, data_list)}</div>
        <br/>
        <div align=center>${await render61_table(metadata, data_list)}</div>
        </div>

    </li>

    <li style="display: inline-block;float:right;width:48%">
        <div align=center>${await render62_chart(p_post_html, metadata2, data_list2)}</div>
        <br/>
        <div align=center>${await render62_table(metadata2, data_list2)}</div>
        </div>
...
async function render61_chart(p_post_html, p_metadata, p_data_list)
{
...
async function render62_chart(p_post_html, p_metadata, p_data_list)
{
    const totals = new Map();
...
async function render61_table(p_metadata, p_data_list)
{
    const totals = new Map();
    ...
    return `<table class="table rounded-0 mb-0"  style="width:100%"
...
async function render62_table(p_metadata, p_data_list)
{
    const totals = new Map();
    const name_to_title = new Map();
