...
async function on_hash_change(e)
{
    if(e.isTrusted)
    {
        ...
        switch(g_ui.url_state.selected_form_name.split('?')[0])
        {
            case 'add-new-user':
                add_new_user_render();
                break;
            case 'edit-user':
                view_edit_user_renderer(g_ui.url_state.selected_form_name.split('?')[1]);
                break;
            case '':
            default:
                load_values();
...
function edit_user_click(p_user_id)
{
    ...
    window.location.href = set_url_hash(`edit-user?${p_user_id.split(":")[1]}`);
    //show_hide_user_management_back_button(true);
}
