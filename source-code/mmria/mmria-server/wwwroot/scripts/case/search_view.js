function search_text_change(p_form_control)
{
    ...
    updateUrlFromSelectValue

    if(search_text != null && search_text.length > 3)
    {
        //form_tag.innerHTML = "Search results for: <em>" + search_text + "</em>";
        form_dropdown.value = "";
        form_container.innerHTML = "Search results for: <em>" + search_text + "</em><br/><br/>" + render_search_text(g_metadata, "", search_text).join("");
    }
    else
    {
        if(form_dropdown.value.length > 0)
        {
            ...
            //form_container.innerHTML = page_render(g_metadata, g_data, g_ui, "g_metadata", "g_data", "", false, post_html_call_back).join("");
