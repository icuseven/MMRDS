        ...
        case "grid":
            ...
            {*/
                for(let i = 0; i < p_ctx.data.length; i++)
                {
                    ...
                    //let new_context = get_seach_text_context(p_ctx.result, p_ctx.post_html_render, p_ctx.metadata, child_data, p_ctx.mmria_path + "/" + i + "/", p_ctx.metadata_path  + ".children[" + i + "]", p_ctx.object_path, p_ctx.search_text);
                    //render_search_text(new_context);
    ...
    if(style_object == null)
    {
        ...
        switch (p_ctx.metadata.type.toLocaleLowerCase())
        {
            case "date":
                ...
                p_ctx.post_html_render.push('  {');
                p_ctx.post_html_render.push('              var elem = document.querySelector("#' + convert_object_path_to_jquery_id(p_ctx.object_path) + ' input.date "); elem.value = p_value;');
                p_ctx.post_html_render.push('              g_set_data_object_from_path("' + p_ctx.object_path + '", "' + p_ctx.metadata_path + '", "' + p_ctx.dictionary_path + '", p_value);');
                p_ctx.post_html_render.push('  }');
                p_ctx.post_html_render.push('});');
                break;
            case "datetime":
