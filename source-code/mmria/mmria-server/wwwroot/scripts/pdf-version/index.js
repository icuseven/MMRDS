...
function GetTableHeader(p_result, p_node) 
{
    ...
    {
		case "TH":
			p_result.push(safe_decodeURI(p_node.textContent).trim());
			break;
		case "TR":
		default:
...
function ConvertHTMLDOMWalker(p_result, p_node) 
{
	...
	switch (p_node.nodeName.toUpperCase()) 
    {
		case "TABLE":
			...
			break;
		case "#TEXT":
			p_result.push({ text: safe_decodeURI(p_node.textContent).trim().replace("<br>", "\n") });
			return;
			break;
		case "P":
		case "DIV":
        ...
        case "SPAN":
            ...
            {
                p_result.push({ text: ' ' + safe_decodeURI(span_text_array[0].text),  style: convert_attribute_to_pdf(p_node) });
            }
            else
            {
        ...
        case "B":
			...
			p_result.push({ text: safe_decodeURI(p_node.textContent).trim(), style: convert_attribute_to_pdf(p_node, strong_attr) });
			return;
			break;
		case "BR":
			p_result.push({ text: "\n" });
		...
		case "EM":
			...
			p_result.push({ text: safe_decodeURI(p_node.textContent).trim(), style: convert_attribute_to_pdf(p_node, em_attr) });
			return;
			break;
		case "UL":
			let ul_array = [];
		...
		case "LI":
            ...
            {
				let li_node = { text: safe_decodeURI(p_node.textContent).trim() }
						...
						for (let j = 0; j < p_metadata.children.length; j++) {
							...
							if (p_data[i][child.name] != null) {
								if (child.type == 'list') {
									...
									row.push({ text: safe_decodeURI(p_data[i][child.name]), style: ['tableDetail'], },);
								}
							}
							else {
